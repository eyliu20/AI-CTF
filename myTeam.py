...
class CTFExtractor(FeatureExtractor):
    ...
    def getFeatures(self, state, action, thisAgent):
        ...
        ePDict = {enemies[0]:enemy1Position, enemies[1]:enemy2Position}
        ePacmen = []
        eGhosts = []
        ...
        for g in eGhosts:
            if nextAgentState.isPacman and isWithin1Block(ePDict[g], myNextPos) :
                if(nextState.getAgentState(g).scaredTimer > 0):
                    features["num-scared-ghosts-1-away"] += 1
                else:
                    features["num-danger-ghosts-1-away"] += 1
            if myNextPos == ePDict[g] and nextAgentState.isPacman:
                if nextState.getAgentState(g).scaredTimer > 0:
        ...
        for p in ePacmen:
            if not nextAgentState.isPacman and isWithin1Block(ePDict[p], myNextPos):
                if nextAgentState.scaredTimer > 0:
                    features["num-super-pac-1-away"] += 1
                else:
                    features["num-edible-pac-1-away"] += 1
            if (not nextAgentState.isPacman) and myNextPos == ePDict[p]:
                if nextAgentState.scaredTimer > 0:
                    features["busted"] = 1
                else:
                    features["pacmen-eaten"] += 1
        ...
        features["enemy-food-left-after"] = len(thisAgent.getFood(nextState).asList()) / 10
        features["my-food-left-after"] = len(thisAgent.getFoodYouAreDefending(nextState).asList()) / 10
        
        if enemyCapsules != []:
            features["distance-to-enemy-capsule"] = thisAgent.distances.getDistance(myPosition, enemyCapsules[0]) / 30
...
def isWithin1Block(pos1, pos2):
    return distanceCalculator.manhattanDistance(pos1, pos2) == 1
